...
describe 'Favicons test' do
  ...
  it 'fails for broken internal favicon' do
    ...
    expect(proofer.failed_tests.first).to match(/internal favicon asdadaskdalsdk.png/)
  end

  it 'fails for broken external favicon' do
    broken = "#{FIXTURES_DIR}/favicon/external_favicon_broken.html"
    proofer = run_proofer(broken, :file, checks: ['Favicon'])
    expect(proofer.failed_tests.first).to match(%r{External link https://www.github.com/asdadaskdalsdk.png})
  end

  it 'fails for ignored with url_ignore' do
    ignored = "#{FIXTURES_DIR}/favicon/internal_favicon_broken.html"
    proofer = run_proofer(ignored, :file, checks: ['Favicon'], url_ignore: [/asdadaskdalsdk/])
    expect(proofer.failed_tests.first).to match(/no favicon specified/)
  end

  it 'translates links via url_swap' do
    translated_link = "#{FIXTURES_DIR}/favicon/internal_favicon_broken.html"
    proofer = run_proofer(translated_link, :file, checks: ['Favicon'], url_swap: { /^asdadaskdalsdk.+/ => '../resources/gpl.png' })
